...
from utils.nli_processor import NLIProcessor

wandb.init(project="MED_NLI")
...
if __name__ == "__main__":
    ...
    args.device = device
    args.n_gpu = n_gpu
    args.num_labels = NLIProcessor.NUM_CLASSES
    args.is_multilabel = NLIProcessor.IS_MULTILABEL
    args.best_model_dir = f"./temp/model_{timestamp_str}/"
    # Record config on wandb
    wandb.config.update(args)
    print_args_as_table(args)

    processor = NLIProcessor(args.data_dir)
    ...
    for i in range(args.repeat_runs):
        ...
        model.classifier = torch.nn.Linear(config.hidden_size, args.num_labels)
        ...
        if (
            test_result[0]["test_accuracy"] < 0.84
