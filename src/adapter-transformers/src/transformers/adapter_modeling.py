...
class BertFusion(nn.Module):
    ...
    def __init__(self, config, n_adapters=None):
        ...
        if self.config.adapter_fusion["temperature"]:
            self.T = self.config.adapter_fusion["temperature"]
            if self.T < 0:
                self.moe = MoE(
                    input_size=config.hidden_size,
                    output_size=config.hidden_size,
                    num_experts=self.n_adapters,
                    k=self.T * -1,
                    noisy_gating=True,
                )
        else:
    ...
    def forward(self, query, key, value, residual):
        ...
        if self.T < 0:
            attention_probs, _ = self.moe.noisy_top_k_gating(query_layer.unsqueeze(2), train=True)
            attention_probs = attention_probs.squeeze(2)
        elif self.T == 1.0:
            attention_probs = F.softmax(attention_scores, dim=-1)
        else:
            attention_probs = F.gumbel_softmax(attention_scores, tau=self.T, dim=-1)
