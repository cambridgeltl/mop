...
class MoE(nn.Module):
    ...
    def _prob_in_top_k(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        threshold_positions_if_in = (torch.arange(batch) * m + self.k).to(clean_values.device)
        ...
        prob_if_in = Normal(
            torch.tensor([0.0]).to(clean_values.device), torch.tensor([1.0]).to(clean_values.device)
        ).cdf((clean_values - threshold_if_in) / noise_stddev)
        prob_if_out = Normal(
            torch.tensor([0.0]).to(clean_values.device), torch.tensor([1.0]).to(clean_values.device)
        ).cdf((clean_values - threshold_if_out) / noise_stddev)
